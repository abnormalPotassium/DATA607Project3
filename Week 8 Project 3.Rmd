...
nyc_jobs_combination[,3:5] <- lapply(nyc_jobs_combination[,3:5], tolower)
...
In order to be able to truly relate the keywords within each column to other information within the dataset, we will need to create new columns on the dataset that detects matches for each data science skill category. This will be stored in boolean format.

```{r keyword mutation}
# Create the new columns in the data frame
for (category_name in names(data_science_categories)) {
  nyc_jobs_combination[[category_name]] <- as_tibble(sapply(data_science_categories[[category_name]],grepl,apply(nyc_jobs_combination[, 3:5], 1, paste, collapse = " "))) %>%
    mutate(!!category_name := if_any(where(is.logical)), .keep="none") %>%
    pull(!!category_name)
}
