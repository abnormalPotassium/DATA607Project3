...
We can also breakdown the data by category and see data acquisiton skills are most in demand within our data.
...
```{r dataframe keyword integration 3}
...
```{r dataframe skill split analyze 1}
df1 <- split_skills |>
...
For the second part of our data analysis we take a slightly different approach to looking into keyword matches. We only care about what category each match comes from and total up the keywords coming from those categories into columns.
